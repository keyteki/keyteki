describe('Grappling Tentacle', function () {
    describe("Grappling Tentacle's ability", function () {
        beforeEach(function () {
            this.setupTest({
                player1: {
                    house: 'keyraken',
        ...
        it('archives when put into play without friendly legendary keyraken in play', function () {
            ...
            this.player1.scrap(this.grapplingTentacle);
        ...
        it('deals 3 damage to legendary keyraken when destroyed', function () {
            ...
            this.player2.play(this.wailOfTheDamned);
            this.player2.clickCard(this.grapplingTentacle);
            expect(this.grapplingTentacle.location).toBe('discard');
            expect(this.legendaryKeyraken.tokens.damage).toBe(3);
            expect(this.player2).isReadyToTakeAction();
        });

        it('captures 3 amber on play', function () {
            this.player1.playCreature(this.legendaryKeyraken0);
            this.player1.playCreature(this.grapplingTentacle);
            expect(this.grapplingTentacle.tokens.amber).toBe(3);
