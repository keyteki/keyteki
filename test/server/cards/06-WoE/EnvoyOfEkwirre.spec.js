    ...
    describe("Envoy of Ekwirrĕ's ability", function () {
        beforeEach(function () {
            this.setupTest({
                player1: {
                    house: 'ekwidon',
                    inPlay: ['troll', 'envoy-of-ekwirre', 'umbra', 'pelf'],
        ...
        it('when using should prompt to pick a creature to swap everything with', function () {
            ...
            expect(this.player1.player.cardsInPlay[1]).toBe(this.envoyOfEkwirre);
            ...
            this.envoyOfEkwirre.tokens.amber = 2;
            this.envoyOfEkwirre.tokens.damage = 1;

            // trigger the action
            this.player1.reap(this.envoyOfEkwirre);
            this.player1.clickCard(this.troll);
            // positions have swapped
            expect(this.player1.player.cardsInPlay[0]).toBe(this.envoyOfEkwirre);
            ...
            expect(this.envoyOfEkwirre.tokens.amber).toBe(5);
            expect(this.envoyOfEkwirre.tokens.damage).toBe(3);
        });
        ...
        it('should kill the swapped creature if there is too much damage', function () {
            ...
            expect(this.envoyOfEkwirre.location).toBe('discard');
            expect(this.troll.tokens.amber).toBe(2);
            expect(this.troll.tokens.damage).toBe(1);
        ...
        it('should swap various counters', function () {
            ...
            this.envoyOfEkwirre.addToken('growth', 4);

            this.player1.reap(this.envoyOfEkwirre);
            this.player1.clickCard(this.troll);

            expect(this.troll.tokens.growth).toBe(4);
            expect(this.envoyOfEkwirre.tokens.ward).toBe(1);
            expect(this.envoyOfEkwirre.tokens.doom).toBe(1);
        });
        ...
        it('should swap upgrades', function () {
            ...
            expect(this.envoyOfEkwirre.power).toBe(10);
        });
    });
});
