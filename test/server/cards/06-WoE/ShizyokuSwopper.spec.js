describe('Shĭzyokŭ Swopper', function () {
    describe("Shĭzyokŭ Swopper's ability", function () {
        ...
        it('should swap with the defender, if both survived the fight', function () {
            this.player1.fightWith(this.shizyokuSwopper, this.mother);
            expect(this.player2.player.cardsInPlay).toContain(this.shizyokuSwopper);
            expect(this.player1.player.cardsInPlay).not.toContain(this.shizyokuSwopper);
        ...
        it('should not swap with the defender, if the defender was destroyed.', function () {
            this.player1.fightWith(this.shizyokuSwopper, this.helperBot);
            expect(this.player1.player.cardsInPlay).toContain(this.shizyokuSwopper);
            expect(this.player2.player.cardsInPlay).not.toContain(this.shizyokuSwopper);
            expect(this.helperBot.location).toBe('discard');
        });

        it('should not swap with the defender, if the Swopper was destroyed.', function () {
            this.player1.fightWith(this.shizyokuSwopper, this.troll);
            ...
            expect(this.shizyokuSwopper.location).toBe('discard');
        });
    });
});
