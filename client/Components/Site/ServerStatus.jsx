...
const ServerStatus = props => {
    ...
    if(connected) {
        ...
        if(responseTime === undefined) {
            ...
            pingText = <React.Fragment><span>{ serverType }: </span><span className={ pingClass }>{ responseTime }ms</span></React.Fragment>;
        }
    } else if(connecting) {
        className += ' glyphicon-remove-circle text-warning';
        toolTip += ' connecting';
        pingText = <React.Fragment><span>{ serverType }: </span><span className='text-warning'>{ t('Connecting') }</span></React.Fragment>;
    } else {
        className += ' glyphicon-ban-circle text-danger';
        toolTip += ' disconnected';
        pingText = <React.Fragment><span>{ serverType }: </span><span className='text-danger'>{ t('Disconnected') }</span></React.Fragment>;
    }
