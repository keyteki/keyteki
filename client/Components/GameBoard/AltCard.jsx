        ...
        return (
            <div className='card-alt'>
                <div className='card-top-row'>
                    ...
                    { ['upgrade', 'event'].includes(this.props.card.type) && <div className={ `card-faction upgrade thronesicon thronesicon-${this.props.card.faction} with-background` } /> }
                </div>
                ...
                <div className={ classNames('card-name-row', { 'vertical': this.props.card.type === 'artifact' }) }>
                    { this.props.card.strength && <div className='card-strength'>{ this.props.card.strength }</div> }
                    { ['creature', 'artifact'].includes(this.props.card.type) && <div className='card-name'>{ this.props.card.unique ? <span className='card-unique' /> : null } { this.props.card.name }</div> }
                    { ['creature', 'artifact'].includes(this.props.card.type) && <div className={ `card-faction thronesicon thronesicon-${this.props.card.faction} with-background` } /> }
                </div>
                <div className='card-text'>
                    { ['upgrade'].includes(this.props.card.type) && <div className='card-name'>{ this.props.card.unique ? <span className='card-unique' /> : null } { this.props.card.name }</div> }
                </div>
            </div>
        );
    }
