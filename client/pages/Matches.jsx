...
class Matches extends React.Component {
    ...
    computeWinner(game) {
        if(!game.winner || (game.winner === game.players[0].name) || (game.winner === game.players[1].name)) {
            return game.winner;
        }
    ...
    render() {
        ...
        if(this.props.apiLoading) {
            content = <div><Trans>Loading matches from the server...</Trans></div>;
        } else if(!this.props.apiSuccess) {
            content = <AlertPanel type='error' message={ this.props.apiMessage } />;
        } else {
            let matches = this.props.games ? this.props.games.map(game => {
                ...
                return (
                    <tr key={ game.gameId }>
                        ...
                        <td style={ { 'white-space': 'nowrap' } }>{ myKeys } x { oppKeys }</td>
                        <td style={ { 'white-space': 'nowrap' } }>{ t(game.gameType) }</td>
                        <td style={ { 'white-space': 'nowrap' } }>{ t(game.gameFormat) }</td>
                        <td style={ { 'white-space': 'nowrap' } }>{ moment(game.startedAt).format('YYYY-MM-DD HH:mm') }</td>
                        <td style={ { 'white-space': 'nowrap' } }>{ duration.get('minutes') }m { duration.get('seconds') }s</td>
                    </tr>
                );
            }) : null;

            let table = (this.props.games && this.props.games.length === 0) ? <div>You have no recorded matches.</div> : (
                <table className='table table-striped'>
                    <thead>
                        <tr>
                            <th><Trans>My Deck</Trans></th>
                            <th><Trans>Opponent</Trans></th>
                            <th><Trans>Opponent&apos;s Deck</Trans></th>
                            <th><Trans>Winner</Trans></th>
                            <th><Trans>Reason</Trans></th>
                            <th><Trans>Keys</Trans></th>
                            <th><Trans>Type</Trans></th>
                            <th><Trans>Format</Trans></th>
                            <th><Trans>Started At</Trans></th>
                            <th><Trans>Duration</Trans></th>
                        </tr>
                    </thead>
                    <tbody>
                        { matches }
...
function mapStateToProps(state) {
    return {
        ...
        games: state.games && state.games.games && state.games.games.filter(game => game.players && game.players.length === 2 && game.decks && game.decks.length === 2),
