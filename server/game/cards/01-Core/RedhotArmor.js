        ...
        this.play({
            ...
            then: {
                gameAction: ability.actions.dealDamage(context => ({
                    target: context.preThenEvents.filter(event => event.name === 'onReduceArmor' && event.amount > 0).map(event => event.card),
                    amountForCard: (card) => context.preThenEvents.find(event => event.name === 'onReduceArmor' && event.card === card).amount
                }))
            }
        });
    }
