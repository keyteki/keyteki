        ...
        this.play({
            targets: {
                flank: {
                    ...
                    gameAction: [
                        ...
                        ability.actions.dealDamage(context => {
                            ...
                            return {
                                target: context.targets.flank.neighbors[0].neighbors.find(card => card !== context.targets.flank),
                                amount: 1
                            };
                        })
                    ]
                ...
                neighbor: {
                    dependsOn: 'flank',
                    cardType: 'creature',
                    cardCondition: (card, context) => context.targets.flank.neighbors.length > 1 ? context.targets.flank.neighbors.includes(card) : false,
                    gameAction: [
                        ability.actions.dealDamage({
                            amount: 2
                        }),
                        ability.actions.dealDamage(context => {
                            if(!context.targets.neighbor || context.targets.neighbor.neighbors.length === 0) {
                                return { amount: 0 };
                            }

                            return {
                                target: context.targets.neighbor.neighbors.find(card => card !== context.targets.flank),
                                amount: 1
                            };
                        })
                    ]
            ...
            effectArgs: context => {
                ...
                return [context.targets.flank, ', 2 damage to ', neighbor, ' and 1 damage to ', neighbor.neighbors.find(card => card !== context.targets.flank)];
            }
        });
    }
}
