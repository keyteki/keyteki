            ...
            if(typeof(ability) === 'function') {
                ...
                if(filteredAbilities.length > 1) {
                    ...
                    for(let a of filteredAbilities) {
                        const generatingEffect = context.game.effectEngine.effects.find(effect =>
                            effect.effect.getValue(context.source) === a);
                        if(generatingEffect) {
                            choices.push(generatingEffect.source.name);
                            handlers.push(() => ability = generatingEffect.effect.getValue(context.source));
                        } else {
                            let ownAbility = filteredAbilities.find(a => a.card === context.source);
                            if(ownAbility) {
