...
const optionsDefinition = [
    { name: 'locale-dir', type: String, defaultValue: path.join(__dirname, '..', '..', 'public', 'locales') },
...
function verifyLocale(localeDir, sourceLanguage, targetLanguage) {
    ...
    if(sourceLocaleLength !== targetLocaleLength) {
        console.log('>> Some keys are missing. ' + sourceLanguage + ' has ' + sourceLocaleLength + ' entries, while ' + targetLanguage + ' has ' +
            targetLocaleLength + ' entries.');
    ...
    if(options['verbose'] === '2') {
        for(let key of sourceLocaleKeys) {
            if(sourceLocale[key] === targetLocale[key]) {
                console.log('W: value for key [' + key + '] are equal, confirm: ' + sourceLocale[key]);
            }
        }
    }
}
