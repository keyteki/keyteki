...
const languages = ['en', 'zhhant', 'zhhans', 'pt', 'fr', 'es', 'de', 'it', 'pl', 'th'];
...
const buildArchon = (card) => new Promise(cardResolve => {
    ...
    Promise.all(promises).then(([cardBack, house1, house2, house3]) => {
        ...
        const path = Path.resolve(__dirname, './images', `${ Buffer.from(card.join()).toString('base64') }.png`);
...
const buildAllFiles = async () => {
    new registerFont('./public/fonts/Oswald-Regular.ttf', {family: 'Keyforge'});
    new registerFont('./public/fonts/ZCOOL-Regular.ttf', {family: 'Keyforge'});
    new registerFont('./public/fonts/Kanit-Regular.ttf', {family: 'Keyforge'});
    const cards = permute();
